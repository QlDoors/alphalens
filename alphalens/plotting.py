...
def plot_quantile_statistics_table(factor_data):
    numeric_data = factor_data[['factor_quantile', 'factor']]
    quantile_stats = numeric_data.groupby('factor_quantile') \
...
def plot_ic_hist(ic, ax=None):
    ...
    for a, (period_num, ic) in zip(ax, ic.items()):
        ...
        # UPDATE TO sns.distplot(ic.replace(np.nan, 0.), norm_hist=True, ax=a)
        sns.histplot(ic.replace(np.nan, 0.), kde=True, stat='density', ax=a)
